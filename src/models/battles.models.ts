 ...
 * @returns Number
 */
const calculateDamage = (player: CharacterWithStats): number => {
  try {
 ...
 * @returns Number
 */
const calculateSpeed = (player: CharacterWithStats): number => {
  try {
 ...
 */
export const battle = (cha1Id: string, cha2Id: string): string => {
  try {
    const character1: CharacterWithStats | undefined = getCharacterById(cha1Id);
    const character2: CharacterWithStats | undefined = getCharacterById(cha2Id);
    ...
    let attacker: CharacterWithStats | undefined;
    let defender: CharacterWithStats | undefined;
    let attackerSpeed: number;
    let defenderSpeed: number;
    let winner: CharacterWithStats | undefined;

    log.push(
      `Battle between ${character1.name} (${character1.job}) - ${character1.hp} HP and ${character2.name} (${character2.job}) - ${character2.hp} HP begins!`
    );

    //this will run untill 1 character dead
    while (!isDead) {
      const character1Speed: number = calculateSpeed(character1);
      const character2Speed: number = calculateSpeed(character2);
      ...
      log.push(
        `${attacker.name} (${attacker.job}) with ${attackerSpeed} speed was faster than ${defender.name} (${defender.job}) with ${defenderSpeed} speed and will attack first.`
      );
      ...
      if (!isDead) {
        ...
        log.push(
          `${defender.name} attacks ${attacker.name} back for ${counterDamage}, ${attacker.name} has ${attacker.hp} HP remaining.`
        );
      }
    }
